...
class Task {
    ...
    deleteTask(todoIndex, taskIndex) {
        modules_todo.getTodos()[todoIndex].tasks.splice(taskIndex, 1);
    };
};

/* harmony default export */ const modules_task = (Task);
...
class Todo {
    ...
    addTask(title, description, dueDate, priority) {
        const newTask = new modules_task(title, description, dueDate, priority);
...
/* harmony default export */ const modules_todo = (new TodoList());
...
function storeTodos() {
    const todos = modules_todo.getTodos();
...
function getStoredTodos() {
    ...
    getTodos.forEach(todo => {
        ...
        modules_todo.todos.push(todo);
        Object.setPrototypeOf(todo, Todo.prototype);
        todo.tasks.forEach(task => {
            Object.setPrototypeOf(task, modules_task.prototype);
        });
    });
};
...
function createAddTodoModal() {
    ...
    addTodoForm.addEventListener("submit", (e) => {
        ...
        modules_todo.addTodo(todoName, "Incomplete");
        renderTodos();
...
function createEditTodoModal(index) {
    const todo = modules_todo.getTodos()[index];
    // Create modal
...
function createAddTaskModal(todoIndex) {
    ...
    addTaskForm.addEventListener("submit", (e) => {
        ...
        modules_todo.getTodos()[todoIndex].tasks.push(new modules_task(taskTitle, taskDesc, taskDue, taskPrio));
        // Set default incomplete task
        modules_todo.getTodos()[todoIndex].status = "Incomplete";
        // Re-add active todo
        renderTodos();
        const todoItems = document.querySelectorAll(".todo-item");
...
function renderTodos() {
    ...
    modules_todo.getTodos().forEach((todo, todoIndex) => {
...
function TodoDeleteHandler(index) {
    if (confirm("Are you sure you want to delete this todo?")) {
        modules_todo.deleteTodo(modules_todo.getTodos()[index]);
        renderTodos();
        renderTasks();
        renderInfo();
        storeTodos();
...
function renderTasks() {
    ...
    todoItems.forEach((item, todoIndex) => {
        if (item.classList.contains("active-todo")) {
            ...
            sectionTitle.textContent = `Tasks (${modules_todo.getTodos()[todoIndex].name})`;
            // Create add task btn
            ...
            modules_todo.getTodos()[todoIndex].tasks.forEach((task, taskIndex) => {
                // Create task divs
                ...
                // Format the distance
                let resultDate = formatDistance_formatDistance(now, dueDate, { includeSeconds: true });
                ...
                taskCheckbox.addEventListener("change", () => {
                    ...
                    modules_todo.getTodos()[todoIndex].status = allChecked ? "Completed" : "Incomplete";
                    renderTodos();
                    const todoItems = document.querySelectorAll(".todo-item");
                    todoItems[todoIndex].classList.add("active-todo");
                    renderInfo();
...
function taskDeleteHandler(todoIndex, taskIndex) {
    if (confirm("Are you sure you want to delete this task?")) {
        modules_todo.getTodos()[todoIndex].tasks[taskIndex].deleteTask(todoIndex, taskIndex);
        renderTodos();
        const todoItems = document.querySelectorAll(".todo-item");
        todoItems[todoIndex].classList.add("active-todo");
        renderTasks();
